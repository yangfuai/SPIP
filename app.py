...
rng = {'alpha_LE':"-36° ~ 36°", 'XC':"0.23 ~ 0.90", 'C':"-0.20 ~ 0.20",
               'd2z_C_XC':"-5 ~ 0", 'd3z_C_XC':"-25 ~ 25", 'z_TE':"-0.05 ~ 0.05",
               'alpha_TE':"-36° ~ 36°", 'R_LE':"0.003 ~ 0.25", 'XT':"0.23 ~ 0.65",
               'T':"0.05 ~ 0.6", 'd2t_XT':"-5 ~ 0", 'd3t_XT':"-25 ~ 25",
...
with tab1:
    ...
    if uploaded_file is not None:
        ...
        # 原始 vs 拟合对比图
        st.markdown("### 原始翼型vs拟合对比")
        fig1, ax1 = plt.subplots(figsize=(8, 6))
        ax1.plot(UpX, UpY, 'b-', lw=3)
        ax1.plot(LowX, LowY, 'b-', lw=3)
        x_fit, zu_fit, zl_fit, _, _ = SPIP_fit(opt_params)
        ax1.plot(x_fit, zu_fit, 'r--', lw=2.5)
        ax1.plot(x_fit, zl_fit, 'r--', lw=2.5)
        ax1.set_xlabel('x/c', fontsize=14); ax1.set_ylabel('z/c', fontsize=14)
        ax1.grid(True, alpha=0.3); ax1.set_aspect('equal', adjustable='box')
        #ax1.set_title("Original airfoil vs SPIP fitting airfoil", fontsize=15, pad=15)
        ...
        if st.button("下载当前翼型坐标 (.dat 格式)", type="primary"):
            try:
                x_dl = np.concatenate([x_new[::-1], x_new[1:]])
                y_dl = np.concatenate([zl_new[::-1], zu_new[1:]])
                content = f"{Path(uploaded_file.name).stem}_SPIP.dat\n"
                for x, y in zip(x_dl, y_dl):
                    content += f"{x:.10f}  {y:.10f}\n"
                st.download_button("点击下载 .dat 文件", content,
                                   file_name=f"{Path(uploaded_file.name).stem}_SPIP.dat", mime="text/plain")
            except:
                st.error("当前参数无效")
            ...
            if len(valid_df) > 0:
                ...
                with zipfile.ZipFile(buffer, "w", zipfile.ZIP_DEFLATED) as zf:
                    for _, row in valid_df.iterrows():
                        ...
                        try:
                            x, zu, zl, _, _ = SPIP_fit(p)
                            content = f"Airfoil_{int(row['ID']):05d}\n"
                            for xx, yy in zip(np.concatenate([x[::-1], x]), np.concatenate([zl[::-1], zu])):
